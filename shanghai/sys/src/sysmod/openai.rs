...
pub enum OutputElement {
    ...
    WebSearchCall(WebSearchCall),
    /// Reasoning generated by a reasoning model.
    Reasoning(Reasoning),
}
...
/// Reasoning output returned by a reasoning model.
///
/// The reasoning content is not part of the user-facing answer. It is kept in
/// the response so that the complete Responses API output can be deserialized,
/// but callers should normally ignore it and use [`ResponseObject::output_text`]
/// for the answer.
#[allow(dead_code)]
#[derive(Clone, Debug, Deserialize)]
...
mod tests {
    ...
    fn load_test_config() -> config::Config {
        let config_path = utils::dir::config_dir().unwrap().join("config.toml");
    ...
    fn test_parse_reasoning_output() {
        ...
        let output: OutputElement = serde_json::from_value(value).unwrap();
        assert!(matches!(output, OutputElement::Reasoning(_)));
    }
    ...
    async fn simple_assistant() {
        let _unset = config::set(load_test_config());
