...
mut でない static で内部を変更可能にするため Cell を使うと、
...
`Mutex` や `RwLock` を使うことになるのでそれでスレッドセーフになるし、
`OnceLock` の中身にさらに `Mutex` や `RwLock` を入れるよりは
`Option<T>` で nullable みたいにして自力でやればいいんじゃないですかね。。

```rust
// グローバル変数としてそのまま公開するよりは、
// グローバル関数のローカルスコープに閉じ込めて関数を公開したほうがよさそう
static GLOBAL: Mutex<Option<MyData>> = Mutex::new(None);
```
