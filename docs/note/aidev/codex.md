...
Codex が起動したら、自然言語で AI に質問やお願いができるが、
スラッシュから始まるコマンドを打つと特殊な設定等を呼び出せる。

* `/model`
  * 使用モデルを切り替える。モデルごとに性能と値段が違う。
  * **デフォルトでは値段の高い最新モデルが選択されている場合があるので注意。**
* `/status`
  * 現在の設定一覧や、トークン使用量を確認できる。
* `/clear`
  * 会話履歴(コンテキスト)をリセットする。
  * API を自分で叩いていると分かるが、入力トークン列=人間とAIのここまでの対話内容、
    出力トークン列=AIの最新返答、である。
    この2つにモデルごとの係数を掛けたものが課金額である。
    会話を続けていると O(n^2) のオーダで課金額が増えていくので
    こまめなリセットがおすすめ。
  * API 従量課金ではなく月額サブスクリプションではダイレクトには響かないかもしれないが、
    使用制限等はトークン数で決まるはず。
* `/compact`
  * 上記のコンテキスト増大問題に対する回答の一つ。
  * これまでの対話内容を(AIを使って)要約して短くする。AI は要約も得意。
  * これ自体にもお金がかかるが、以後の入力
* `/copy`
  * 最後の AI 返答をコピーする。
