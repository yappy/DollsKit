...
BODY_LIMIT = 65_000
RETRY_COUNT = 4
BEAD_ID_PATTERN = re.compile(r"^Beads issue: `([^`\n]+)`$", re.MULTILINE)
...
def bead_id_from_github_issue(issue: dict[str, Any]) -> str | None:
    """Extract the Beads ID from a body generated by this synchronizer."""
    body = issue.get("body")
    if not isinstance(body, str) or MARKER not in body:
        return None
    match = BEAD_ID_PATTERN.search(body)
    return match.group(1) if match else None


def github_issues(options: Options) -> list[dict[str, Any]]:
    issues = run_json(
        gh_command(
            [
                "issue",
    ...
    if not isinstance(issues, list):
        raise RuntimeError("The result of gh issue list is not an array")
    return issues


def reconcile_external_refs(
    issues: Iterable[dict[str, Any]],
    github_items: Iterable[dict[str, Any]],
    options: Options,
) -> None:
    """Recover missing refs from GitHub bodies before creating new Issues.

    Older workflow runs created GitHub Issues but failed to commit the
    corresponding ``external_ref`` values to Dolt.  Generated bodies contain
    the stable Beads ID, so the oldest surviving Issue is safe to adopt as the
    canonical one.  Existing duplicate Issues are intentionally left alone.
    """
    candidates: dict[str, list[dict[str, Any]]] = {}
    for github_item in github_items:
        issue_id = bead_id_from_github_issue(github_item)
        url = github_item.get("url")
        if not issue_id or not isinstance(url, str):
            continue
        candidates.setdefault(issue_id, []).append(github_item)

    for issue in issues:
        if external_issue_url(issue):
            continue
        issue_id = value(issue, "id")
        matches = candidates.get(issue_id, [])
        if not matches:
            continue
        canonical = min(matches, key=lambda item: int(item.get("number", 0)))
        url = str(canonical["url"])
        github_issue_api_path(url)
        command = ["bd", "update", issue_id, "--external-ref", url]
        if options.dry_run:
            print(f"DRY-RUN REUSE {issue_id}: {url}")
        else:
            run(command)
            suffix = f" ({len(matches)} candidates)" if len(matches) > 1 else ""
            print(f"REUSED {issue_id}: {url}{suffix}")
...
def main() -> int:
    ...
    current = bead_issues()
    reconcile_external_refs(current, github_issues(options), options)
    sync_beads(options)
    issues = bead_issues()
